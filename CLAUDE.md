...
Domain truth lives in Spec Kit memory (`knowledge/`, catalogs), not in agent skill files. Spec Kit skills are workflow glue only (`/speckit-specify`, `/speckit-plan`, `/speckit-tasks`, `/speckit-implement`, …). Update matching knowledge/catalog files in the same change set when behavior changes (see sync map in `project-context.md`).
