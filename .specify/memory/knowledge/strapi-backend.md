...
**Copy and paste:** With a block selected, Copy (or ⌘/Ctrl+C while focus is inside the composition surface, and not in a text field) stores that block and its nested children. Paste (or ⌘/Ctrl+V) inserts a fresh copy of that subtree as the last child of the block selected afterward. Nest allowlists and depth caps still apply; every copied node gets a new composition id. The original stays where it is.
