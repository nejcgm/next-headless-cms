...
**Admin live preview:** Strapi 5.44's native Content Manager preview is enabled for `page` (`config/admin.ts` → `admin.preview`). The `handler` resolves the edited document's `slug` (`strapi.documents('api::page.page').findOne(...)`) and returns a URL through the **same** `/api/preview` route above — no second preview mechanism. Requires backend env `PREVIEW_BASE_URL` (the tenant frontend's origin, e.g. `http://localhost:3002`) alongside `PREVIEW_SECRET` (same value as the frontend's). The preview reflects the editor's saved draft — there is no autosave; an earlier debounced-autosave attempt was reverted after it was found to silently drop edits made while a background save was in flight. See `specs/011-page-builder-polish/contracts/live-preview.md`.
