...
| `blocks.flex` | `flex` | `maxDepth: 5`. `direction?` (`row`/`column` — also the sole replacement for the retired `stack`), `gap?` (sizing string), `align?` (`start`/`center`/`end`/`stretch`/`baseline`), `justify?` (`start`/`center`/`end`/`between`/`around`/`evenly`), `wrap?`, slim box styles, **`slots` (json)** |
...
| `blocks.text` | `text` | Leaf: `content`, `as?` (enum `p`/`span`/`h1`–`h6`, replaces the old `variant` step enum), `fontSize?`/`lineHeight?`/`letterSpacing?` (plain numbers, px), `bold?`, `uppercase?`, slim box styles (`color`, …) minus `fontSize`/`height`/`minWidth`/`minHeight`/`maxHeight` — titles use `as="h_"` + size + bold, not a separate heading type |
