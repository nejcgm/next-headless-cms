...
**`flex`** — `maxDepth: 5`. `direction` (`row`/`column` — the sole replacement for the retired `stack`), `gap` (sizing string), `align` (`start`/`center`/`end`/`stretch`/`baseline`), `justify` (`start`/`center`/`end`/`between`/`around`/`evenly`), `wrap`. New Flex nodes write those renderer defaults onto the node (`row` / `"16"` / `center` / `start` / `false`). Optional `backgroundImage` (covers the flex box), `overlay` (0–1 black scrim on the image only), and `anchorId`.
...
**`text`** — leaf (`maxDepth: 1`). `content`, `as` (enum `p`/`span`/`h1`–`h6`, replaces the old `variant` step enum — renders via `createElement(as, …)`), `fontFamily` (enum `body`/`heading`/`display`, added in the redesign pass — see "Multiple fonts" below), `fontSize` / `lineHeight` / `letterSpacing` as plain **numbers** (px; replaces the old named-step `fontSize` enum + implicit `lineHeight: 1.625`), `bold`, `uppercase` (new), plus the rest of box styles minus `fontSize`/`height`/`minWidth`/`minHeight`/`maxHeight` (omitted — text sizes itself via `as` + explicit numbers). Former heading roles are plain `text` with `as="h_"` + `fontSize`/`bold`. Stats = a column `flex` of two `text` nodes (was `stack`).

**Multiple fonts (design redesign pass)**: `text.fontFamily` (`body`/`heading`/`display`) applies `font-body`/`font-heading`/`font-display` (Tailwind `fontFamily` utilities, `tailwind.config.ts`) — omitted, a Text node just inherits whatever font cascades down (historically always `font-body`, since nothing upstream of it in a composed page ever set `font-heading`; that gap is exactly why headings never actually looked different from body copy — see the seed-script note below). This is a real per-node author choice, not a global site setting — unlike `ThemeTokens.fonts`, which only controls *what each of those three names points to*, not which nodes use which name. Vukan's Bike wires a third distinct face — `display: "var(--font-oswald)"` (Oswald, condensed/athletic) — alongside the existing Montserrat `heading` / Inter `body` pair, loaded in the shared `src/app/layout.tsx` next to the other two (see Theme tokens below).
...
**Theme tokens (`ThemeTokens`, `src/core/types/tenant.ts`)**: `colors.{primary, secondary, accent, background, foreground, muted, border, textPrimary, mutedForeground}`, `fonts.{heading, body, display?}`, `borderRadius`. `ThemeProvider` (`src/core/theme/provider.tsx`) emits each as a `--color-*` / `--font-*` / `--radius` CSS variable on its wrapper div — this is a **fixed, hand-maintained list**; a new named token must be added to both `ThemeTokens` and the provider or it is silently inert (this is exactly what happened to `mutedForeground` before it was added — the `text` primitive's non-`color`-overridden state, plus the footer/accordion/header locale switcher, all read `--color-muted-foreground`, which was hardcoded in `globals.css` and unthemeable per tenant until this field existed). `fonts.heading` / `fonts.body` / `fonts.display` are plain CSS `font-family` values — set them to `var(--font-<name>)` only if `src/app/layout.tsx` actually loads that font via `next/font` with a matching `variable` name; otherwise the reference is dead and text falls through to the browser default. `fonts.display` is **optional** on the `ThemeTokens` type (tenants that don't set it just never emit `--font-display`, and `text.fontFamily: "display"` falls back to the `font-display` Tailwind stack's next entry, `var(--font-heading)`) — `resort-example` doesn't set it.
