...
**Page authoring:** Editors compose the page body in the Strapi **page composition surface**: block tree on the left, a draft preview of the public page in the center, and property fields on the right. Unsaved composition edits show in that preview and are not stored until Save. Slug, `lang`, template, and SEO stay on the native Page form. Nested JSON is a collapsed technical fallback only. Navigation and products are unchanged.
